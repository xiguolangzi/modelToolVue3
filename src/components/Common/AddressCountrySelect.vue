...
// 1 api 接口
import { listConfigAddressCountry } from "@/api/models/configAddressCountry.js";
...
const actualPlaceholder = computed(() => {
  return props.placeholder || "国家编码/国家名称"
});
...
const fetchData = async (query = "") => {
  ...
  try {
    ...
    if (/^[A-Z]{1,3}$/.test(query)) {
      // 1-3位大写字母（省份编码）
      queryParams.countryCode = query;
    } else {
      // 其他情况（省份名称）
      queryParams.nameZh = query;
    }

    console.log("请求参数:", queryParams);

    // 4 发起请求
    const response = await listConfigAddressCountry(queryParams);
    dataList.value = response.rows.map(item => ({
      // 5 同步视图参数
      value: item.countryCode,
      label: `${item.countryCode} - ${item.nameZh}`,
